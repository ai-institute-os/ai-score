...
class CustomerApplication(Base):
    ...
    agent_business_summary: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    agent_target_audience: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    agent_products_services: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    agent_market_context: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    agent_competitor_notes: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    agent_fit_recommendation: Mapped[Optional[str]] = mapped_column(String, nullable=True)
    ...
    # Per-application placeholder overrides applied before standard substitutions in PDF render
    text_overrides: Mapped[Optional[dict]] = mapped_column(JSON, nullable=True)
